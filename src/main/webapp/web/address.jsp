        ...
        <div class="rs_content">
        	<form method="post" action="${actionUrl}">
	            ...
	            <div class="recipients">
	                ...
	                <input type="text" name="recvName" id="receiverName" value="${address.recvName}"/>
	            </div>
	            <!--收货人所在城市等信息-->
	            <div data-toggle="distpicker" class="address_content">
					 <span class="red">*</span><span class="kuan">省&nbsp;份：</span>
					 <select id="provinces" name="recvProvince" style="width:130px;">
					  ...
					  城&nbsp;市：
					<select id="cities" name="recvCity" style="width:130px;">
						<option value="0">----- 请选择 -----</option>
						<c:forEach items="${cities}" var="c">
							<option value="${c.code}"
								<c:if test="${c.code == address.recvCity}">
								 selected="selected"
								</c:if>
							>${c.name}</option>
						</c:forEach>
					  ...
					  区/县：
					<select id="areas" name="recvArea" style="width:130px;">
						<option value="0">----- 请选择 -----</option>
	            ...
	            <div class="address_particular">
	                ...
	                <textarea name="recvAddress"  id="receiverAddress" cols="60" rows="3" placeholder="建议您如实填写详细收货地址">${address.recvAddress}</textarea>
	            </div>
	            <!--收货人地址-->
	            ...
	            <div class="address_name">
	                ...
	                <input type="text" id="addressName" name="tag" value="${address.tag}"/>如：
	                <span class="sp">家</span><span class="sp">公司</span><span class="sp">宿舍</span>
	            </div>
	            <!--保存收货人信息-->
	            <div>
	            	<input type="hidden" name="action" value="${action}"/>
	            	<input type="hidden" name="id" value="${address.id}"/>
	                <input type="submit" id="save" class="button" value="${actionTitle}收货人信息"/>
	                <c:if test="${address != null}">
	                	<input type="button" class="button" value="返回添加"/>
	                </c:if>
	            </div>
                ...
                <c:forEach items="${addresses}" var="address">
	                ...
	                ">
	                    ...
	                    <span class="operation operation_normal">
	                    	...
	                    	<a href="delete.do?id=${address.id}" class="aco_delete" onclick="return confirm('确认删除吗？')">删除</a>
	                    </span>
	                    <span class="swmr swmr_normal">                	
		                	<c:if test="${address.isDefault == 0}">
		                		<a href="set_default.do?id=${address.id}" class="swmr swmr_normal">设为默认</a>
		                	</c:if>
	                    </span>
	                </div>
                </c:forEach>
