...
public class AddressServiceImpl implements IAddressService{
	...
	public Address addNew(Address address) {	
		...
		//判断当前用户的收货地址的数量
		if(addresses.size() == 0) {//在执行此次增加之前，该用户还没有任何收货地址信息
			// 则此次增加的收货地址应该是默认的
			address.setIsDefault(1);
		}else {//在执行此次增加之前，该用户已经有收货地址信息
			// 则此次增加的收货地址应该是非默认的
	...
	public Address getAddressById(Integer id) {
		String where = "id="+id;
		List<Address> addresses = addressMapper.select(where, null, null, null);
		if(addresses.size() == 0) {
			return null;
		}else {
	...
	public Integer deleteAddressById(Integer id,Integer uid) {
		...
		if (data.getIsDefault() == 1) {
			...
			List<Address> list = addressMapper.select(where, orderBy, offset, countPerPage);
			// 判断是否找到了数据
			if (list.size() > 0) {
				// 找到了数据，则取出第1条，
				// 这条数据就是删除后，
	...
	public Integer changeAddressInfo(String username,Address address) {		
		//获取省市区的名称
		String district = getDistrict(address);
		address.setRecvDistrict(district);
		//确定参数address中的必要数据
	...
	private String getDistrict(Address address) {
		...
		district.append(provinceService.getProvinceByCode(provinceCode).getName());
		district.append(cityService.getCityByCode(cityCode).getName());
		district.append(areaService.getAreaByCode(areaCode).getName());
